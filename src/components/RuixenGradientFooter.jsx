...
export function RuixenGradientFooter({
  ...
  const rawId = useId();
  const uid = rawId ? rawId.replace(/:/g, "") : "ruixen-grad";
  ...
  useEffect(() => {
    ...
    // on a real page and inside iframe preview alike.
  ...
  return (
    ...
    <footer
      className={`relative bg-rak-slate-950 text-rak-slate-300 overflow-hidden ${className}`}
      style={{ paddingBottom: gradientHeight, ...style }}
    >
      {/* Footer Content */}
      <div className="relative z-10">{children}</div>

      {/* Ruixen Viewport-Pinned Rainbow Glow SVG */}
      <div
        ref={bandRef}
        aria-hidden
        style={{
